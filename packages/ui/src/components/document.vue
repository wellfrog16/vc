    ...
    <div class="usage" :class="[$style.main]">
        ...
        <ElTable v-if="data.params" :data="data.params" row-key="param" :span-method="spanMethod">
            ...
            <ElTableColumn prop="type" label="类型" min-width="100">
                <template #default="{ row }">
                    <ElLink v-if="row.type && row.type.includes('http')" type="primary" target="_blank" :href="row.type">参考链接</ElLink>
                    <ElLink v-if="row.type && row.type.includes('./')" type="primary" :href="row.type">查看</ElLink>
                    <span v-if="row.type && !row.type.includes('http') && !row.type.includes('./')">{{ row.type }}</span>
                </template>
            </ElTableColumn>
        ...
        <ElTable v-if="data.events" :data="data.events">
            ...
            <ElTableColumn prop="param" label="回调参数">
                <template #default="{ row }">
                    <span v-if="!row.param.includes('http')">{{ row.param }}</span>
...
function spanMethod({ row, _column, _rowIndex, columnIndex }: any) {
